...
wd=$(pwd)
echo "working dir:" $wd
EXEC="python -u hmm_mixed.mapplwm.py"
...
file_name="hmm_mixed.mapplwm.${param_list}"

echo ${param_list}

echo
echo "Running MAPPL-WM"
echo
{
...
hyperfine --runs "$hyperfine_runs" \
          ...
          --output inherit \
          --ignore-failure \
          "$EXEC --config MAPPL-WM --num_samples_exponent 0 --alpha ${alpha}  --beta ${beta}  --init 1 --horizon ${horizon} --seed 0 --sigma ${sigma} --theta ${theta}"
